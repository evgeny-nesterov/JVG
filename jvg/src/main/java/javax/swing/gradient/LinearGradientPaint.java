...
/**
 * The <code>LinearGradientPaint</code> class provides a way to fill a {@link java.awt.Shape} with a linear color gradient pattern. The user may
 * specify 2 or more gradient colors, and this paint will provide an interpolation between each color. The user also specifies start and end
 * points which define where in user space the color gradient should begin and end.
 * <p>
 * The user must provide an array of floats specifying how to distribute the colors along the gradient. These values should range from 0.0 to 1.0
 * and act like keyframes along the gradient (they mark where the gradient should be exactly a particular color).
 * <p>
 * For example: <br>
 * <code>
 * <p>
 ...
 * </code>
 *<p>
 * This code will create a LinearGradientPaint which interpolates between red and white for the first 20% of the gradient and between white and
 * blue for the remaining 80%.
 * <p>
 * In the event that the user does not set the first keyframe value equal to 0 and the last keyframe value equal to 1, keyframes will be created
 * at these positions and the first and last colors will be replicated there. So, if a user specifies the following arrays to construct a
 * gradient:<br>
 ...
 * <p>
 * The user may also select what action the LinearGradientPaint should take when filling color outside the start and end points. If no cycle
 * method is specified, NO_CYCLE will be chosen by default, so the endpoint colors will be used to fill the remaining area.
 * <p>
...
public final class LinearGradientPaint extends MultipleGradientPaint {
	...
	/**
	 * <p>
	 * Constructs an <code>LinearGradientPaint</code> with the default NO_CYCLE repeating method and SRGB colorspace.
	 * 
	 * @param startX
	 ...
	 *             if start and end points are the same points, or if fractions.length != colors.length, or if colors is less than 2 in size.
	 */
	...
	public LinearGradientPaint(float startX, float startY, float endX, float endY, float[] fractions, Color[] colors, CycleMethodEnum cycleMethod, AffineTransform transform) {
		this(new Point2D.Float(startX, startY), new Point2D.Float(endX, endY), fractions, colors, cycleMethod, SRGB, transform);
	}

	// public LinearGradientPaint(LinearGradient gradient)
	// {
	// this(gradient.getX1(), gradient.getY1(), gradient.getX2(),
	// gradient.getY2(),
	...
	/**
	 * <p>
	 * Constructs a <code>LinearGradientPaint</code> with the default NO_CYCLE repeating method and SRGB colorspace.
	 * 
	 * @param start
	...
	public LinearGradientPaint(LinearGradient gradient, int x, int y, int w, int h) {
		this(x + w * gradient.getX1(), y + h * gradient.getY1(), x + w * gradient.getX2(), y + h * gradient.getY2(), gradient.getFractions(), gradient.convertColors(), gradient.getCycleMethod());
	}
	 ...
	 * 
	 * @return a {@link Point2D} object that is a copy of the point that anchors the first color of this <code>LinearGradientPaint</code>.
	 */
	 ...
	 * 
	 * @return a {@link Point2D} object that is a copy of the point that anchors the last color of this <code>LinearGradientPaint</code>.
	 */
	...
	public static void main(String[] args) {
		...
		f.setContentPane(new JLabel() {
			...
			public void paint(Graphics g) {
				...
				Color[] colors = { new Color(255, 0, 0, 200), Color.white, Color.blue };
				Paint p = new LinearGradientPaint(start, end, dist, colors);
