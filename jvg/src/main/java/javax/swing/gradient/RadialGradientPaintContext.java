...
/**
 * A RadialGradienPaint only supports circular gradients, but it should be possible to scale the circle to look approximately elliptical, by
 * means of a gradient transform passed into the RadialGradientPaint constructor.
 */
final class RadialGradientPaintContext extends MultipleGradientPaintContext {
	...
	/**
	 * This code works in the simplest of cases, where the focus == center point, the gradient is noncyclic, and the gradient lookup method is
	...
	private void calculateFixedPointSqrtLookupTable() {
		float sqStepFloat;
		sqStepFloat = ((fastGradientArraySize * fastGradientArraySize) / (MAX_PRECISION - 2));
	...
	/**
	 * Fill the raster, cycling the gradient colors when a point falls outside of the perimeter of the 100% stop circle. This calculation first
	 * computes the intersection point of the line from the focus through the current point in the raster, and the perimeter of the gradient
	 * circle. Then it determines the percentage distance of the current point along that line (focus is 0%, perimeter is 100%). Equation of a
	 * circle centered at (a,b) with radius r: (x-a)^2 + (y-b)^2 = r^2 Equation of a line with slope m and y-intercept b y = mx + b replacing y
	 * in the cirlce equation and solving using the quadratic formula produces the following set of equations. Constant factors have been
	 * extracted out of the inner loop.
	 */
	private void cyclicCircularGradientFillRaster(int pixels[], int off, int adjust, int x, int y, int w, int h) {
		...
		final float precalc3 = -2 * centerX; // const in inner loop quad.
		// formula
	...
	/**
	 * Fill the raster, cycling the gradient colors when a point falls outside of the perimeter of the 100% stop circle. Use the anti-aliased
	 * gradient lookup. This calculation first computes the intersection point of the line from the focus through the current point in the
	 * raster, and the perimeter of the gradient circle. Then it determines the percentage distance of the current point along that line (focus
	 * is 0%, perimeter is 100%). Equation of a circle centered at (a,b) with radius r: (x-a)^2 + (y-b)^2 = r^2 Equation of a line with slope m
	 * and y-intercept b y = mx + b replacing y in the cirlce equation and solving using the quadratic formula produces the following set of
	 * equations. Constant factors have been extracted out of the inner loop.
	 */
	private void antiAliasFillRaster(int pixels[], int off, int adjust, int x, int y, int w, int h) {
		...
		double pixSzSq = (a00 * a00 + a01 * a01 + a10 * a10 + a11 * a11);
