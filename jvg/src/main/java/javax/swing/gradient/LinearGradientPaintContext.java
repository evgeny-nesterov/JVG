...
final class LinearGradientPaintContext extends MultipleGradientPaintContext {
	 ...
	 * @param t
	 *            the {@link AffineTransform} from user space into device space (gradientTransform should be concatenated with this)
	 * @param hints
			...
			if (dgdX == 0) {
				...
				if (dgdX > 0) {
					...
					while (off < gradLimit) {
						...
						int steps = (int) Math.ceil((fractions[gradIdx + 1] - g) / dgdX);
