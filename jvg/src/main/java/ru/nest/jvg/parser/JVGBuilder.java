...
public class JVGBuilder {
	...
	public static String className = "ru.nest.jvg.parser.JVGBuilder";

	public static JVGBuilder create() {
		try {
			Class<JVGBuilder> clazz = (Class<JVGBuilder>) Class.forName(className);
	...
	public byte[] buildByteArray(JVGComponent[] components) throws JVGParseException {
	...
	public String buildUTF(JVGComponent[] components) throws JVGParseException {
		return build(components, "UTF8");
	}
	...
	public void build(JVGComponent[] components, Element componentsElement) throws JVGParseException {
	...
	protected void buildComponent(JVGComponent component, Element componentElement) throws JVGParseException {
	...
	protected void setScripts(JVGComponent component, Element componentElement) {
	...
	protected void setScripts(Element jvgElement) {
		if (pane != null) {
			Element scriptsElement = new Element("scripts");
			boolean thereAreScripts = false;
	...
	protected void setProperties(JVGComponent component, Element componentElement) {
	...
	protected void setTransform(AffineTransform transform, Element componentElement) {
	...
	protected String getImage(Resource<? extends Icon> image) throws JVGParseException {
		if (image != null) {
	...
	protected boolean setForm(JVGShape component, Element formElement) throws JVGParseException {
	...
	protected boolean setTextDocument(javax.swing.text.StyledDocument doc, Element e) throws JVGParseException {
		try {
			int start = 0;
	...
	protected String getFillValue(Resource resource) throws JVGParseException {
		Object o = resource.getResource();
		if (o instanceof Color) {
			return getColor(resource);
		} else if (o instanceof LinearGradient) {
			return "lg(" + getLinearGradient(resource, true) + ")";
		} else if (o instanceof RadialGradient) {
			return "rg(" + getRadialGradient(resource, true) + ")";
	...
	protected void setCharacterAttributes(javax.swing.text.AttributeSet attr, Element characterElement) throws JVGParseException {
	...
	protected static StringBuffer appendStyle(String name, boolean append, StringBuffer buf) {
		if (append) {
			if (buf == null) {
	...
	protected void setParagraphAttributes(javax.swing.text.AttributeSet attr, Element paragraph) {
	...
	protected void setPainters(JVGShape shape, Element componentElement) throws JVGParseException {
	...
	protected boolean setPaint(Painter painter, Element paintElement) throws JVGParseException {
	...
	protected boolean setDraw(Painter painter, Element paintElement) throws JVGParseException {
		if (painter != null) {
	...
	protected boolean setDraw(Resource drawResource, Element drawElement) throws JVGParseException {
		...
		if (drawObject instanceof Color) {
			...
			String gradient = getLinearGradient(drawResource, true);
			...
			String gradient = getRadialGradient(drawResource, true);
	...
	protected String getColor(Resource<Color> resource) {
		if (resource != null) {
	...
	protected String getTexture(Resource<Texture> resource) throws JVGParseException {
		if (resource != null) {
			String id = resource.getName();
			if (id != null) {
				resources.addResource(resource);
	...
	protected String getFont(Resource<Font> font) {
		if (font != null) {
	...
	protected <S extends Stroke> String getStroke(Resource<S> stroke) {
		if (stroke != null) {
	...
	protected String getType(JVGComponent component) {
		return types.get(component.getClass());
	}

	private static char[] curveChars = { 'M', 'L', 'Q', 'C', 'X' };

	protected void setPath(PathIterator iter, Element e) {
	...
	protected void setControl(JVGCustomActionArea control, Element componentElement) {
	}
	...
	public static void main(String[] args) {
		try {
			...
			JVGBuilder builder = new JVGBuilder();
			String s = builder.buildUTF(root.getChildren());
