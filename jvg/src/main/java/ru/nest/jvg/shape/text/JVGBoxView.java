...
public class JVGBoxView extends CompositeView {
	public JVGBoxView(Element elem, int axis) {
		super(elem);
	...
	void updateChildSizes() {
		...
		if (majorAxis == X_AXIS) {
			for (int i = 0; i < n; i++) {
				View v = getView(i);
				v.setSize(majorSpans[i], minorSpans[i]);
			}
		} else {
			for (int i = 0; i < n; i++) {
				View v = getView(i);
				v.setSize(minorSpans[i], majorSpans[i]);
			}
		}
	}
	...
	public void paint(Graphics g, Shape allocation) {
		...
		for (int i = 0; i < n; i++) {
			...
			int crx0 = clip != null ? clip.x : 0, crx1 = crx0 + (clip != null ? clip.width : 0);
			int cry0 = clip != null ? clip.y : 0, cry1 = cry0 + (clip != null ? clip.height : 0);
	...
	public float getPreferredSpan(int axis) {
		...
		if (axis == majorAxis) {
			return majorRequest.preferred + marginSpan;
		} else {
			return minorRequest.preferred + marginSpan;
		}
	}
	...
	public float getMinimumSpan(int axis) {
		...
		float marginSpan = (axis == X_AXIS) ? getLeftInset() + getRightInset() : getTopInset() + getBottomInset();
		if (axis == majorAxis) {
			return majorRequest.minimum + marginSpan;
		} else {
			return minorRequest.minimum + marginSpan;
		}
	}
	...
	public float getMaximumSpan(int axis) {
		...
		float marginSpan = (axis == X_AXIS) ? getLeftInset() + getRightInset() : getTopInset() + getBottomInset();
		if (axis == majorAxis) {
			return majorRequest.maximum + marginSpan;
		} else {
			return minorRequest.maximum + marginSpan;
		}
	}
	...
	// TODO for all views width is forced to width of the text pane!
