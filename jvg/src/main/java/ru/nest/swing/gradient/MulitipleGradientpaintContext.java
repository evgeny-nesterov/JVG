...
/**
 * This is the superclass for all PaintContexts which use a multiple color gradient to fill in their raster. It provides the actual color
 * interpolation functionality. Subclasses only have to deal with using the gradient to fill pixels in a raster.
 * 
...
abstract class MultipleGradientPaintContext implements PaintContext {
	...
	/**
	 * This boolean specifies wether we are in simple lookup mode, where an input value between 0 and 1 may be used to directly index into a
	 * single array of gradient colors. If this boolean value is false, then we have to use a 2-step process where we have to determine which
	 * gradient array we fall into, then determine the index into that array.
	 */
	protected boolean isSimpleLookup = true;

	/**
	 * This boolean indicates if the gradient appears to have sudden discontinuities in it, this may be because of multiple stops at the same
	...
	/**
	 * Array which contains the interpolated color values for each interval, used by calculateSingleArrayGradient(). It is protected for possible
	...
	/**
	 * This holds the blend of all colors in the gradient. we use this at extreamly low resolutions to ensure we get a decent blend of the
	 * colors.
	 */
	...
	/**
	 * Maximum length of the fast single-array. If the estimated array size is greater than this, switch over to the slow lookup method. No
	 * particular reason for choosing this number, but it seems to provide satisfactory performance for the common case (fast lookup).
	 */
	private static final int MAX_GRADIENT_ARRAY_SIZE = 5000;

	/**
	 * Constructor for superclass. Does some initialization, but leaves most of the heavy-duty math for calculateGradient(), so the subclass may
	 * do some other manipulation beforehand if necessary. This is not possible if this computation is done in the superclass constructor which
	...
	public MultipleGradientPaintContext(ColorModel cm, Rectangle deviceBounds, Rectangle2D userBounds, AffineTransform t, RenderingHints hints, float[] fractions, Color[] colors, MultipleGradientPaint.CycleMethodEnum cycleMethod, MultipleGradientPaint.ColorSpaceEnum colorSpace) throws NoninvertibleTransformException {
		...
		for (int i = 0; i < fractions.length - 1; i++) {
			...
			if (normalizedIntervals[idx] < 0) {
				normalizedIntervals[idx] = 0;
			}
			idx++;
		}
	...
	/**
	 * This function is the meat of this class. It calculates an array of gradient colors based on an array of fractions and color values at
	 * those fractions.
	 */
	...
	/**
	 * FAST LOOKUP METHOD This method calculates the gradient color values and places them in a single int array, gradient[]. It does this by
	 * allocating space for each interval based on its size relative to the smallest interval in the array. The smallest interval is allocated
	 * 255 interpolated values (the maximum number of unique in-between colors in a 24 bit color system), and all other intervals are allocated
	 * size = (255 * the ratio of their size to the smallest interval). This scheme expedites a speedy retrieval because the colors are
	 * distributed along the array according to their user-specified distribution. All that is needed is a relative index from 0 to 1. The only
	 * problem with this method is that the possibility exists for the array size to balloon in the case where there is a disproportionately
	 * small gradient interval. In this case the other intervals will be allocated huge space, but much of that data is redundant. We thus need
	...
	/**
	 * SLOW LOOKUP METHOD This method calculates the gradient color values for each interval and places each into its own 255 size array. The
	 * arrays are stored in gradients[][]. (255 is used because this is the maximum number of unique colors between 2 arbitrary colors in a 24
	 * bit color system) This method uses the minimum amount of space (only 255 * number of intervals), but it aggravates the lookup procedure,
	 * because now we have to find out which interval to select, then calculate the index within that interval. This causes a significant
	 * performance hit, because it requires this calculation be done for every point in the rendering loop. For those of you who are interested,
	 * this is a classic example of the time-space tradeoff.
	 */
	...
	/**
	 * Yet another helper function. This one extracts the color components of an integer RGB triple, converts them from LinearRGB to SRGB, then
	...
	/**
	 * Helper function to index into the gradients array. This is necessary because each interval has an array of colors with uniform size 255.
	 * However, the color intervals are not necessarily of uniform length, so a conversion is required.
	 * 
	 * @param position
	 *            the unmanipulated position. want to map this into the range 0 to 1
	...
	/**
	 * Helper function to index into the gradients array. This is necessary because each interval has an array of colors with uniform size 255.
	 * However, the color intervals are not necessarily of uniform length, so a conversion is required. This version also does anti-aliasing by
	...
	/**
	 * Took this cacheRaster code from GradientPaint. It appears to recycle rasters for use by any other instance, as long as they are
	 * sufficiently large.
	 */
	protected final static synchronized WritableRaster getCachedRaster(ColorModel cm, int w, int h) {
		if (cm == cachedModel) {
			if (cached != null) {
	 ...
	 * sufficiently large.
	 */
	protected final static synchronized void putCachedRaster(ColorModel cm, WritableRaster ras) {
		if (cached != null) {
