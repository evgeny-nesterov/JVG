...
public class CloseTabbedPaneUI extends BasicTabbedPaneUI implements SwingConstants {
	...
	/*
	 * protected ChangeListener tabChangeListener; protected PropertyChangeListener propertyChangeListener; protected MouseListener
	 * mouseListener; protected FocusListener focusListener;
	 */
	...
	/*
	 * In an attempt to preserve backward compatibility for programs which have extended CloseTabbedPaneUI to do their own layout, the UI uses
	 * the installed layoutManager (and not tabLayoutPolicy) to determine if scrollTabLayout is enabled.
	 */
	...
	protected void installKeyboardActions() {
		...
		/*
		 * if (scrollableTabLayoutEnabled()) { tabScroller.scrollForwardButton.setAction (am.get("scrollTabsForwardAction"));
	...
	/*
	 * protected void uninstallKeyboardActions() { SwingUtilities.replaceUIActionMap(tabPane, null); SwingUtilities.replaceUIInputMap(tabPane,
	 * JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT, null); SwingUtilities.replaceUIInputMap(tabPane, JComponent.WHEN_FOCUSED, null); }
	 */

	/**
	 * Reloads the mnemonics. This should be invoked when a memonic changes, when the title of a mnemonic changes, or when tabs are
	 * added/removed.
	 */
	...
	/**
	 * Paints the tabs in the tab area. Invoked by paint(). The graphics parameter must be a valid <code>Graphics</code> object. Tab placement
	 * may be either: <code>JTabbedPane.TOP</code>, <code>JTabbedPane.BOTTOM</code>, <code>JTabbedPane.LEFT</code>, or
	 * <code>JTabbedPane.RIGHT</code>. The selected index must be a valid tabbed pane tab index (0 to tab count - 1, inclusive) or -1 if no tab
	 * is currently selected. The handling of invalid parameters is unspecified.
	 * 
	 * @param g
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects, int tabIndex, Rectangle iconRect, Rectangle textRect) {
		...
		paintTabBackground(g, tabPlacement, tabIndex, tabRect.x, tabRect.y, tabRect.width, tabRect.height + 1, isSelected);

		paintTabBorder(g, tabPlacement, tabIndex, tabRect.x, tabRect.y, tabRect.width, tabRect.height + 1, isSelected);
		...
		if (tabIndex == currentCloseRolloverIndex) {
			...
			g.drawRect(closeIconRect.x - 2, closeIconRect.y - 2, closeIconRect.width + 3, closeIconRect.height + 3);
		}
	...
	/*
	 * This method will create and return a polygon shape for the given tab rectangle which has been cropped at the specified cropline with a
	 * torn edge visual. e.g. A "File" tab which has cropped been cropped just after the "i": ------------- | ..... | | . | | ... . | | . . | | .
	 * . | | . . | -------------- The x, y arrays below define the pattern used to create a "torn" edge segment which is repeated to fill the
	 * edge of the tab. For tabs placed on TOP and BOTTOM, this righthand torn edge is created by line segments which are defined by coordinates
	 * obtained by subtracting xCropLen[i] from (tab.x + tab.width) and adding yCroplen[i] to (tab.y). For tabs placed on LEFT or RIGHT, the
	 * bottom torn edge is created by subtracting xCropLen[i] from (tab.y + tab.height) and adding yCropLen[i] to (tab.x).
	 */
	...
	protected void layoutLabel(int tabPlacement, FontMetrics metrics, int tabIndex, String title, Icon icon, Rectangle tabRect, Rectangle iconRect, Rectangle textRect, boolean isSelected) {
		...
		SwingUtilities.layoutCompoundLabel(tabPane, metrics, title, icon, SwingConstants.CENTER, SwingConstants.LEFT, SwingConstants.CENTER, SwingConstants.TRAILING, tabRect, iconRect, textRect, textIconGap);
	...
	protected void paintText(Graphics g, int tabPlacement, Font font, FontMetrics metrics, int tabIndex, String title, Rectangle textRect, boolean isSelected) {
		...
		if (v != null) {
			...
			if (tabPane.isEnabled() && tabPane.isEnabledAt(tabIndex)) {
				...
				BasicGraphicsUtils.drawStringUnderlineCharAt(g, title, mnemIndex, textRect.x, y);

			}

			else { // tab disabled
				g.setColor(tabPane.getBackgroundAt(tabIndex).brighter());
				BasicGraphicsUtils.drawStringUnderlineCharAt(g, title, mnemIndex, textRect.x, textRect.y + metrics.getAscent() - 1);
	...
	protected void paintContentBorderBottomEdge(Graphics g, int tabPlacement, int selectedIndex, int x, int y, int w, int h) {
		...
		if (tabPlacement == BOTTOM) {
			Rectangle rect = rects[selectedIndex];
			g.setColor(activeColor);
			g.drawLine(rect.x + 1, y + h - 1, rect.x + rect.width - 2, y + h - 1);
		}
	...
	private void ensureCurrentLayout() {
		...
		/*
		 * If tabPane doesn't have a peer yet, the validate() call will silently fail. We handle that by forcing a layout if tabPane is still
		 * invalid. See bug 4237677.
		 */
		if (!tabPane.isValid()) {
	...
	/**
	 * Returns the bounds of the specified tab in the coordinate space of the JTabbedPane component. This is required because the tab rects are
	 * by default defined in the coordinate space of the component where they are rendered, which could be the JTabbedPane (for WRAP_TAB_LAYOUT)
	 * or a ScrollableTabPanel (SCROLL_TAB_LAYOUT). This method should be used whenever the tab rectangle must be relative to the JTabbedPane
	 * itself and the result should be placed in a designated Rectangle object (rather than instantiating and returning a new Rectangle each
	 * time). The tab index parameter must be a valid tabbed pane tab index (0 to tab count - 1, inclusive). The destination rectangle parameter
	 * must be a valid <code>Rectangle</code> instance. The handling of invalid parameters is unspecified.
	 * 
	 * @param tabIndex
	...
	/**
	 * Returns the tab index which intersects the specified point in the coordinate space of the component where the tabs are actually rendered,
	 * which could be the JTabbedPane (for WRAP_TAB_LAYOUT) or a ScrollableTabPanel (SCROLL_TAB_LAYOUT).
	 */
	...
	/**
	 * Returns the text View object required to render stylized text (HTML) for the specified tab or null if no specialized text rendering is
	 * needed for this tab. This is provided to support html rendering inside tabs.
	 * 
	 * @param tabIndex
	 *            the index of the tab
	...
	boolean requestFocusForVisibleComponent() {
		...
		if (visibleComponent.isFocusable()) {
			...
			// ************* CHECK THIS **********************************
			if (jComponent.getFocusTraversalPolicy().getDefaultComponent(jComponent).requestFocusInWindow()) {
				return true;
			}
	...
	/*
	 * private static class ScrollTabsForwardAction extends AbstractAction { public void actionPerformed(ActionEvent e) { JTabbedPane pane =
	 * null; Object src = e.getSource(); if (src instanceof JTabbedPane) { pane = (JTabbedPane)src; } else if (src instanceof
	 * ScrollableTabButton) { pane = (JTabbedPane)((ScrollableTabButton)src).getParent(); } else { return; // shouldn't happen }
	 * System.out.println("pane: "+pane.getClass().getName()); System.out.println("ui: "+(pane.getUI()).getClass().getName()); CloseTabbedPaneUI
	 * ui = (CloseTabbedPaneUI)pane.getUI(); if (ui.scrollableTabLayoutEnabled()) { ui.tabScroller.scrollForward(pane.getTabPlacement()); } } }
	 * private static class ScrollTabsBackwardAction extends AbstractAction { public void actionPerformed(ActionEvent e) { JTabbedPane pane =
	 * null; Object src = e.getSource(); if (src instanceof JTabbedPane) { pane = (JTabbedPane)src; } else if (src instanceof
	 * ScrollableTabButton) { pane = (JTabbedPane)((ScrollableTabButton)src).getParent(); } else { return; // shouldn't happen }
	 * CloseTabbedPaneUI ui = (CloseTabbedPaneUI)pane.getUI(); if (ui.scrollableTabLayoutEnabled()) {
							...
							switch (tabPlacement) {
								...
								case RIGHT:
									...
									if (totalTabHeight > th) {
										...
										by = dir == SOUTH ? bounds.height - insets.bottom - bsize.height : bounds.height - insets.bottom - 2 * bsize.height;
									}
									break;
	...
	private class ScrollableTabSupport implements ChangeListener, ActionListener {
	...
	private class ScrollableTabButton extends BasicArrowButton implements UIResource, SwingConstants, MouseListener {
	...
	public class Handler implements MouseListener, MouseMotionListener, FocusListener, PropertyChangeListener, ChangeListener {
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (currentCloseRolloverIndex == -1 && rollOverIndex != -1) {
				...
				Point point = SwingUtilities.convertPoint(tabScroller.tabPanel, x, y, tabPane);

				if (tabPane instanceof SimpleCloseTabbedPane) {
					SimpleCloseTabbedPane pane = (SimpleCloseTabbedPane) tabPane;
					pane.fireTabRollOver(new TabRolloverEvent(pane, rollOverIndex, point.x, point.y));
				}
		...
		public void mouseExited(MouseEvent e) {
			...
			if (tabPane instanceof SimpleCloseTabbedPane) {
				SimpleCloseTabbedPane pane = (SimpleCloseTabbedPane) tabPane;
				pane.fireTabRollOverFinished(new TabRolloverEvent(pane, -1, e.getX(), e.getY()));
			}
		}
	...
	/*
	 * GES 2/3/99: The container listener code was added to support HTML rendering of tab titles. Ideally, we would be able to listen for
	 * property changes when a tab is added or its text modified. At the moment there are no such events because the Beans spec doesn't allow
	 * 'indexed' property changes (i.e. tab 2's text changed from A to B). In order to get around this, we listen for tabs to be added or removed
	 * by listening for the container events. we then queue up a runnable (so the component has a chance to complete the add) which checks the
	 * tab title of the new component to see if it requires HTML rendering. The Views (one per tab title requiring HTML rendering) are stored in
	 * the htmlViews Vector, which is only allocated after the first time we run into an HTML tab. Note that this vector is kept in step with the
	 * number of pages, and nulls are added for those pages whose tab title do not require HTML rendering. This makes it easy for the paint and
	 * layout code to tell whether to invoke the HTML engine without having to check the string during time-sensitive operations. When we have
	 * added a way to listen for tab additions and changes to tab text, this code should be removed and replaced by something which uses that.
	 */
