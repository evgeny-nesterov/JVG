...
public class GlassCapturePanel extends JPanel implements MouseInputListener, MouseWheelListener {
	...
	private void forwardMouseEvent(MouseEvent e) {
		...
		if (e instanceof MouseWheelEvent) {
			...
			Point p = SwingUtilities.convertPoint(mainComponent, e.getX(), e.getY(), target);

			MouseWheelEvent retargeted = new MouseWheelEvent(target, id, e.getWhen(), wheelEvent.getModifiers() | wheelEvent.getModifiersEx(), p.x, p.y, wheelEvent.getClickCount(), wheelEvent.isPopupTrigger(), wheelEvent.getScrollType(), wheelEvent.getScrollAmount(), wheelEvent.getWheelRotation());
	...
	/*
	 * Find the lightweight child component which corresponds to the specified location. This is similar to the new 1.2 API in Container, but we
	 * need to run on 1.1. The other changes are due to Container.findComponentAt's use of package-private data.
	 */
		...
		for (int i = 0; i < ncomponents; i++) {
			...
			// (comp.getPeer() instanceof java.awt.peer.LightweightPeer) &&
				...
				if (comp instanceof Container) {
					...
					Component deeper = findComponentAt(child, x - childLoc.x, y - childLoc.y);
	...
	protected void retargetMouseEvent(int id, MouseEvent e, Component target) {
		...
		MouseEvent retargeted = new MouseEvent(target, id, e.getWhen(), e.getModifiers() | e.getModifiersEx(), p.x, p.y, e.getClickCount(), e.isPopupTrigger());
