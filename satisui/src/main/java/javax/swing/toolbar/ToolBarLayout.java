...
public class ToolBarLayout implements LayoutManager2 {
	...
	private Hashtable<ToolBarConstraints, Component> componentsMap;

	/** The tool bar constraints */
	private ArrayList<ToolBarConstraints> constraintsList;

	/**
	 * <p>
	 * Constructs a new layout instance initialised to the specified number of rows.
	 * 
	 * @param the
	 *            initial number of rows
	 */
	public ToolBarLayout(int toolBarRows) {
		constraintsList = new ArrayList<ToolBarConstraints>();
		componentsMap = new Hashtable<ToolBarConstraints, Component>();
	 ...
	 * 
	 * @param the
	 *            component which needs to be laid out
	 */
	@Override
	public void layoutContainer(Container parent) {
		...
		for (int i = 0; i < compCount; i++) {
			constraints = constraintsList.get(i);
			component = componentsMap.get(constraints);
			...
			if (i != compCount - 1) {
				...
				ToolBarConstraints _constraints = constraintsList.get(i + 1);
				...
				// if the component is resizing
					...
					for (int j = start; j < tbca.length; j++) {
						...
						if (_minWidth == -1) {
							_minWidth = componentsMap.get(constraints).getMinimumSize().width;
						}
						maxWidth -= _minWidth;
	...
	/**
	 * <p>
	 * Returns all the toolbars on the specified row.
	 * 
	...
	private ToolBarConstraints[] getToolBars(int row) {
		...
		Vector<ToolBarConstraints> _toolBars = new Vector<ToolBarConstraints>();

		for (int i = 0, k = constraintsList.size(); i < k; i++) {
			tbc = constraintsList.get(i);
			_row = tbc.getRow();
		...
		return _toolBars.toArray(new ToolBarConstraints[] {});
	}

	/**
	 * <p>
	 * Returns the currently displayed tool bar row count.
	 * 
	...
	/**
	 * <p>
	 * Retrieves the specified components constraints.
	 * 
	 * @param the
	 *            component
	...
	public ToolBarConstraints getConstraint(Component comp) {
		...
		Enumeration<ToolBarConstraints> consEnum = componentsMap.keys();
	...
	/**
	 * <p>
	 * Determines the component count for the specified row.
	 * 
	 * @param the
	 *            row
	...
	private int getComponentCount(int row) {
		...
		for (int i = 0; i < size; i++) {
			constraint = constraintsList.get(i);

			if (constraint.getRow() == row) {
				Component c = componentsMap.get(constraint);
				if (c.isVisible()) {
					count++;
				}
			}
	 ...
	 * @param where
	 *            /how the component is added to the layout.
	 */
	@Override
	...
	private Rectangle getComponentBounds(Component comp, boolean isPreferred) {
		...
		if (rectangle.width <= 0 || rectangle.height <= 0) {
			Dimension dimension = isPreferred ? comp.getPreferredSize() : comp.getMinimumSize();
	...
	protected Dimension getLayoutSize(Container parent, boolean isPreferred) {
		...
		dimension.height = (getRowHeight() * toolBarRows) + BORDER_OFFSET + insets.top + insets.bottom;
		return dimension;
	}
	...
	/**
	 * <p>
	 * Adds the specified component with the specified name to the layout. This does nothing in ToolBarLayout - constraints are required.
	 */
	@Override
	...
	class ToolsPositionComparator implements Comparator<Object> {
		...
		public int compare(Object obj1, Object obj2) {
			...
			int halfWidth = componentsMap.get(cons1).getWidth() / 2;
