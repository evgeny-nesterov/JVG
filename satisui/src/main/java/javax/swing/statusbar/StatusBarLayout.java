...
public class StatusBarLayout implements LayoutManager2, Serializable {
	...
	/**
	 * Creates a new instance of StatusBarLayout with the specified height.
	 * 
	 * @param the
	 ...
	 * <p>
	 * This method is called by AWT when the invalidate method is called on the Container. Since the invalidate method may be called
	 * asynchronously to the event thread, this method may be called asynchronously.
	 * 
	 * @param target
	...
	public void removeLayoutComponent(Component comp) {
		if (componentsMap.containsValue(comp)) {
			for (Enumeration<StatusBarLayoutConstraints> i = componentsMap.keys(); i.hasMoreElements();) {
	...
	/**
	 * Adds the specified component with the specified constraints to the layout. Constraints must be an instance of
	...
	public void addLayoutComponent(Component comp, Object constraints) {
		...
		if (constraints instanceof StatusBarLayoutConstraints) {
			...
			StatusBarLayoutConstraints _constraints = (StatusBarLayoutConstraints) constraints;
	...
	/**
	 * Returns the alignment along the X axis for the container. If the box is horizontal, the default alignment will be returned. Otherwise, the
	...
	/**
	 * Returns the alignment along the Y axis for the container. If the box is vertical, the default alignment will be returned. Otherwise, the
	...
	private Rectangle[] computePositions(Container target) {
		...
		if (resizeCount > 0 && totalComponentWidth < targetDim.width) {
			fillWidth = ((targetDim.width - insets.left - insets.right - totalComponentWidth) / resizeCount);// -
			// 1;
		}
	...
	class ConstraintsComparator implements Comparator<Object> {
		/**
		 * <p>
