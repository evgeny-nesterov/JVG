...
public class GridDockLayoutHelper {
	...
	public static void freePlace(Container container, GridConstraints constr) {
		if (container.getLayout() instanceof GridLayout) {
			GridLayout layout = (GridLayout) container.getLayout();
			for (int i = 0; i < container.getComponentCount(); i++) {
				Component c = container.getComponent(i);
				GridConstraints curConstr = layout.getConstraints(c);

				boolean changed = false;
				boolean intersectX = false;
				if (curConstr.gridx < constr.gridx + constr.gridwidth && curConstr.gridx + curConstr.gridwidth > constr.gridx) {
					if (curConstr.gridx < constr.gridx) {
						int w = constr.gridx - curConstr.gridx;
						if (w > 0) {
							curConstr.gridwidth = w;
							changed = true;
						}
					} else {
						int w = curConstr.gridx + curConstr.gridwidth - (constr.gridx + constr.gridwidth);
						if (w > 0) {
							curConstr.gridx = constr.gridx + constr.gridwidth;
							curConstr.gridwidth = w;
							changed = true;
						}
					}
				...
				if (curConstr.gridy < constr.gridy + constr.gridheight && curConstr.gridy + curConstr.gridheight > constr.gridy) {
					if (curConstr.gridy < constr.gridy) {
						int h = constr.gridy - curConstr.gridy;
						if (h > 0) {
							curConstr.gridheight = h;
							changed = true;
						}
					} else {
						int h = curConstr.gridy + curConstr.gridheight - (constr.gridy + constr.gridheight);
						if (h > 0) {
							curConstr.gridy = constr.gridy + constr.gridheight;
							curConstr.gridheight = h;
							changed = true;
						}
					}
				...
				if (intersectX && intersectY && changed) {
					layout.setConstraints(c, curConstr);
				}
			}
		}
	}
