...
public abstract class MutablePath implements Shape, Cloneable {
	public static final int curvecoords[] = { 2, 2, 4, 6, 0 };
	...
	MutablePath(int rule, int initialTypes) {
		setWindingRule(rule);
	...
	public static class Float extends MutablePath implements Serializable {
		private static final long serialVersionUID = -796613835548990736L;
		...
		public Float(Shape s, AffineTransform at) {
			if (s instanceof MutablePath) {
				MutablePath p2d = (MutablePath) s;
		...
		public final Object clone() {
			return new MutablePath.Float(this);
		}
		...
		static class CopyIterator extends MutablePath.Iterator {
			float floatCoords[];

			CopyIterator(MutablePath.Float p2df) {
				super(p2df);
		...
		static class TxIterator extends MutablePath.Iterator {
			float floatCoords[];

			AffineTransform affine;

			TxIterator(MutablePath.Float p2df, AffineTransform at) {
				super(p2df);
				this.floatCoords = p2df.floatCoords;
				this.affine = at;
			}
		...
		public void deleteSegment(int segmentIndex) {
			...
			int type = pointTypes[segmentIndex];
			if (type == SEG_MOVETO) {
				int mcoord = getCoordIndex(segmentIndex);

				segmentIndex++;
				type = pointTypes[segmentIndex];
				int size = curvecoords[type];
				int coord = getCoordIndex(segmentIndex);

				floatCoords[mcoord] = floatCoords[coord + size - 2];
				floatCoords[mcoord + 1] = floatCoords[coord + size - 1];

				System.arraycopy(pointTypes, segmentIndex + 1, pointTypes, segmentIndex, numTypes - segmentIndex - 1);
				numTypes--;

				if (size > 0) {
					System.arraycopy(floatCoords, coord + size, floatCoords, coord, numCoords - coord - size);
					numCoords -= size;
				}
			} else {
				int size = curvecoords[type];
		...
		public void addSegment(int segmentIndex, int type, double[] coords) {
			...
			if (size != coords.length) {
				throw new RuntimeException("invalid coords length, expected " + size);
			}

			needRoom(false, size);
			System.arraycopy(pointTypes, segmentIndex, pointTypes, segmentIndex + 1, numTypes - segmentIndex);
			pointTypes[segmentIndex] = (byte) type;
			numTypes++;

			if (size > 0) {
				System.arraycopy(floatCoords, coord, floatCoords, coord + size, numCoords - coord);
				System.arraycopy(coords, 0, floatCoords, coord, size);
				numCoords += size;
			}
		}
	...
	public static class Double extends MutablePath implements Serializable {
		...
		@Override
		public final Object clone() {
			return new MutablePath.Double(this);
		}
		...
		static class CopyIterator extends MutablePath.Iterator {
			...
			CopyIterator(MutablePath.Double p2dd) {
				super(p2dd);
		...
		static class TxIterator extends MutablePath.Iterator {
			...
			TxIterator(MutablePath.Double p2dd, AffineTransform at) {
				super(p2dd);
				this.doubleCoords = p2dd.doubleCoords;
				this.affine = at;
			}
		...
		public void deleteSegment(int segmentIndex) {
			...
			if (type == SEG_MOVETO) {
				...
				doubleCoords[mcoord] = doubleCoords[coord + size - 2];
				doubleCoords[mcoord + 1] = doubleCoords[coord + size - 1];

				System.arraycopy(pointTypes, segmentIndex + 1, pointTypes, segmentIndex, numTypes - segmentIndex - 1);
				numTypes--;

				if (size > 0) {
					System.arraycopy(doubleCoords, coord + size, doubleCoords, coord, numCoords - coord - size);
					numCoords -= size;
				}
			} else {
				int size = curvecoords[type];
		...
		public void addSegment(int segmentIndex, int type, double[] coords) {
			...
			if (size > 0) {
				System.arraycopy(doubleCoords, coord, doubleCoords, coord + size, numCoords - coord);
				System.arraycopy(coords, 0, doubleCoords, coord, size);
				numCoords += size;
			}
		}
	...
	public abstract void deleteSegment(int segmentIndex);

	public abstract void addSegment(int segmentIndex, int type, double[] coords);

	public abstract double getCoord(int coordIndex);
	...
	public final synchronized Shape createTransformedShape(AffineTransform at) {
		MutablePath p2d = (MutablePath) clone();
		if (at != null) {
			p2d.transform(at);
		}
		return p2d;
	...
	final void writeObject(java.io.ObjectOutputStream s, boolean isdbl) throws java.io.IOException {
		...
		if (isdbl) {
			dCoords = ((MutablePath.Double) this).doubleCoords;
			fCoords = null;
		} else {
			fCoords = ((MutablePath.Float) this).floatCoords;
			dCoords = null;
		}
	...
	final void readObject(java.io.ObjectInputStream s, boolean storedbl) throws java.lang.ClassNotFoundException, java.io.IOException {
		...
		if (storedbl) {
			((MutablePath.Double) this).doubleCoords = new double[nC];
		} else {
			((MutablePath.Float) this).floatCoords = new float[nC];
		}
	...
	public int getSegmentIndex(int coordIndex) {
		int coord = 0;
		for (int i = 0; i < numTypes; i++) {
			int type = pointTypes[i];
			int size = curvecoords[type];
			if (coord + size > coordIndex) {
				return i;
			}
			coord += size;
		}
	...
	public int getCoordIndex(int segmentIndex) {
		int coord = 0;
