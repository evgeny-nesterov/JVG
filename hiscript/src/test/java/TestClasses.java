...
public class TestClasses extends HiTest {
	...
	public void testClasses() {
		...
		assertSuccess("class C{int c = 1;} assert new C().c == 1;");
		assertSuccess("class C{static int c;} assert C.c == 0;");
		...
		assertSuccess("abstract class C{} new C(){};");
		assertSuccess("abstract class C{void get(){}} new C(){void get(){}};");
		assertSuccess("abstract class C{abstract void get();} new C(){void get(){}};");

		// extends
		assertSuccess("{class B{B(int x){}} new B(1);} {class B{void get(){}} class C extends B{} new C().get();}");
		assertFail("class A {A(boolean x){this(x ? 1 : 0);} A(int y){this(y == 1);}} new A(true);", //
		...
		assertSuccess("static class A{static class B{} static class C extends B{}}");
		...
		// initializing order
		assertSuccess("class A{static String a = B.b + 'A';} class B{static String b = A.a + 'B';} assert A.a.equals(\"nullBA\"); assert B.b.equals(\"nullB\");");
		assertSuccess("class A{static String a = B.b + 'A';} class B{static String b = A.a + 'B';} assert B.b.equals(\"nullAB\"); assert A.a.equals(\"nullA\");");
		...
		// inner classes
		assertSuccess("class A{class B{}} A.B b = new A().new B();");
		assertSuccess("class A{class B{class C{}}} A.B.C c = new A().new B().new C();");
	...
	public void testInnerClasses() {
		// inner class
		assertSuccess("class B{int x = 0; {new A();} class A{A(){x++;}} {new A();}} assert new B().x == 2;");
		assertSuccess("class B{record R(int a);} assert new B().new R(1).a == 1;");
		assertSuccess("static class B{interface I{int get();}} class A implements B.I{public int get(){return 1;}} assert new A().get() == 1;");
		assertSuccess("class B{enum E{a,b,c}} E e = B.E.b; assert e == B.E.b;");
		assertSuccess("class C{} enum E{a} interface I{} record R(int a);");
		...
		assertSuccess("class A {void get(){}} A a = new A(); a.get();");
		...
		// interfaces
		assertSuccess("interface I{interface I1{}} class X implements I {I1 m() {I1 i = new I1(){}; return i;}} assert new X().m() instanceof I.I1;");

		// super class
		assertSuccess("class C1 {class C2{}} class X extends C1 {C2 m() {C2 c2 = new C2(); return c2;}} assert new X().m() instanceof C1.C2;");
		assertSuccess("class C1 {class C2{class C3{}}} class X extends C1 {C1.C2.C3 m() {C1.C2.C3 c3 = new C1().new C2().new C3(); return c3;}} assert new X().m() instanceof C1.C2.C3;");
	}

	@Test
	public void testLocalClasses() {
		assertSuccess("class A{int x; {class B{B(){x = 2;}} new B();}} assert new A().x == 2;");
		assertSuccess("class A {class B{A getA(){return A.this;} A.B getB(){return A.B.this;}}} " + //
				"A a = new A(); assert a.new B().getA() == a; A.B b = a.new B(); assert b. getB() == b;");
		assertSuccess("class A{int x; void m(int x) {class B{B(){A.this.x = x;}} new B();}} A a = new A(); a.m(2); assert a.x == 2;");
		assertSuccess("class A{int x; A m(int y) {class B{B(){x = y;}} new B(); return this;}} assert new A().m(2).x == 2;");
	...
	public void testInheritance() {
		assertSuccess("class A{class A1{int get(){return 1;}} int get(){return 2;}} class B extends A{{assert new A1().get() == 4;} class A1{int get(){return 4;}} {assert new A1().get() == 4;} " //
				+ "int get(){assert new A().get() == 2; assert new A1().get() == 4; class A1{int get(){return 5;}} assert new A1().get() == 5; return 6;}}; assert new B().get() == 6;");
		assertSuccess("class A{int x = 1;} class B extends A{int x = 2; {assert super.x == 1; assert x == 2;} {this.x++; super.x--; assert super.x == 0; assert this.x == 3;}} assert new B().x == 3;");
	}

	@Test
	public void testInterfaces() {
		// extends, implements
		assertSuccess("interface I{int x1 = 1; final static int x2 = 1; abstract void m1(); void m2(); default void m3(){} static void m4(){}}");
		...
		assertFailCompile("interface I{abstract void m(){}}", //
				"';' is expected");
		assertSuccess("interface I1{} interface I2 extends I1{} class C implements I2{} I1 c = new C(); assert c instanceof C; assert c instanceof I1; assert c instanceof I2;");
		assertSuccess("interface I1{} interface I2{} interface I3 extends I1{} interface I12 extends I1, I2{} class C implements I12, I3{} Object c = new C(); assert c instanceof C; assert c instanceof I1; assert c instanceof I2; assert c instanceof I3; assert c instanceof I12;");
		assertSuccess("interface I{void m();} class C implements I{void m(){}}");
		...
		assertFailCompile("interface A extends B{} interface B extends A{}", //
				"class 'B' can not be resolved");
		assertSuccess("interface A{int x = 1;} class B implements A{} assert new B().x == 1;");
		assertSuccess("interface A{int x = 1;} class B implements A{} assert B.x == 1;");
		...
		// initializing order
		assertSuccess("interface A1{int a1 = 1; default int a(){return 0;} interface A2{int a2 = a1 + 1; default int a(){return 0;} interface A3{int a3 = a2 + 1;}}} assert A1.a1 == 1; assert A1.A2.a2 == 2; assert A1.A2.A3.a3 == 3;");
		assertSuccess("interface A1{int a1 = A2.A3.a3 + 1; interface A2{int a2 = a1 + 1; interface A3{int a3 = a2 + 1;}}} assert A1.a1 == 3; assert A1.A2.a2 == 1; assert A1.A2.A3.a3 == 2;");
		assertSuccess("interface A1{int a1 = A2.a2 + 1; interface A2{int a2 = A3.a3 + 1; interface A3{int a3 = a2 + 1;}}} assert A1.a1 == 3; assert A1.A2.a2 == 2; assert A1.A2.A3.a3 == 1;");
		...
		assertSuccess("interface A {default int get(){return 1;}} interface B {default int get(){return 2;}} class C implements A, B{int get(){return B.super.get();}} assert new C().get() == 2;");
		...
		// private methods
		assertSuccess("interface A{private void m(){}}");
		assertSuccess("interface A{private int m(){return 1;} default int get(){this.m(); return m();}} assert new A(){}.get() == 1;");
		...
		// static methods
		assertSuccess("interface A{static int m(){return 1;}} assert A.m() == 1;");
	...
	public void testMethods() {
		...
		assertSuccess("abstract class A{abstract void get();} class C extends A{void get(){}} A a = new C(); a.get();");

		// signature
		assertSuccess("class A{String m(int x, String arg){return \"\";}} String s = new A().m(0, null);");
		...
		assertSuccess("class A{void m(int x, int y){}} class B extends A{void m(int x, byte y){}} new B().m(1, 1);");
		...
		assertSuccess("class C{void m(int... x){assert false;} void m(int x1, int x2, int x3){}} new C().m(1, 2, 3);");
		assertSuccess("class A{int m(int x){return 1;} int m(int x, int y, int z){return 2;} int m(int x, int... y){return 3;}} assert new A().m(1,2) == 3;");

		// TODO check incompatible throws of rewrite method
		assertSuccess("class E1 extends Exception{}; class E2 extends Exception{}; class A{void m() throws E1, E2 {throw new E1();}}");
		...
		// rewrite
		assertSuccess("class A{Number get(){return 1;}} class B extends A{Integer get(){return 2;}}");
		...
		assertSuccess("interface A{Number get();} class B implements A{Integer get(){return 2;}}");
		...
		// synchronized
		assertSuccess("class A{synchronized void m(){int x = 0;}} new A().m();");
	...
	public void testMethodPriority() {
		// varargs
		assertSuccess("class A{void m(int x){} void m(int... x){assert false;} void m(Integer x){assert false;}} new A().m(1);");
		assertSuccess("class A{void m(int x){assert false;} void m(int... x){} void m(Integer x){assert false;}} new A().m(1, 2);");
		assertSuccess("class A{void m(int x){assert false;} void m(int... x){} void m(Integer x){assert false;}} new A().m(new Integer(1), new Integer(2));");
		assertSuccess("class A{void m(int x){assert false;} void m(int... x){assert false;} void m(Integer x){}} new A().m(new Integer(1));");
		assertSuccess("class A{void m(int x, int y){} void m(int... x){assert false;} void m(Integer x, Integer y){assert false;}} new A().m(1, 2);");
		...
		assertSuccess("class A {void m(Float x){assert false;} void m(float x){}} new A().m(1);");
		assertSuccess("class A {void m(Float x){assert false;} void m(double x){}} new A().m(1);");
		assertSuccess("class A {void m(Float x){assert false;} void m(byte x){}} new A().m((byte)1);");
		assertSuccess("class A {void m(Double x){assert false;} void m(double x){}} new A().m(1.1);");

		assertSuccess("class A{void m(int x1, int x2){} void m(int... x){assert false;} void m(Integer x1, Integer x2){assert false;}} new A().m(1, 2);");
		assertSuccess("class A{void m(int x1, int x2){assert false;} void m(int... x){assert false;} void m(Integer x1, Integer x2){}} new A().m(new Integer(1), new Integer(2));");
		assertSuccess("class A{void m(int x1, int x2){assert true;} void m(float x1, float x2){assert false;}} new A().m(1, 2);");
		assertSuccess("class A{void m(int x1, int x2){assert false;} void m(float x1, float x2){assert true;}} new A().m(1, 2f);");
		...
		assertSuccess("class A {void m(int x, int y){assert true;} void m(float x, float y){assert false;}} new A().m(1, 2);"); // priority strict primitives over cast primitives
		assertSuccess("class A {void m(int x, Integer y){assert true;} void m(Integer x, int y){assert false;}} new A().m(1, new Integer(2));"); // priority strict over autoboxing
		assertSuccess("class A {void m(String x, Integer y){assert true;} void m(Object x, Object y){assert false;}} new A().m(\"a\", new Integer(1));"); // priority strict over cast Object
		assertSuccess("class A {void m(Object x, Object y){assert false;} void m(String x, Integer y){assert true;}} new A().m(\"a\", new Integer(1));"); // priority strict over cast Object
		assertSuccess("class A {void m(Double x, Integer y){assert true;} void m(Number x, Number y){assert false;}} new A().m(1.0, 1);"); // priority strict over cast
		assertSuccess("class A {void m(Number x, Number y){assert false;} void m(Double x, Integer y){assert true;}} new A().m(1.0, 1);"); // priority strict over cast
		...
		assertFailCompile("class A{} class B extends A{} class C extends B{} class D{void m(A x, B... y){} void m(B x, A... y){}} new D().m(new B(), new B(), new B());",
				"Ambiguous method call. Both m(A x, B[] y) in D and m(B x, A[] y) in D match");
		assertSuccess("class A{} class B extends A{} class C extends B{} class D{void m(A x, A... y){assert false;} void m(B x, A... y){}} new D().m(new B(), new B(), new B());");
		...
		assertSuccess("class A{} class B extends A{} class C extends B{} class D{void m(A x, A... y){assert false;} void m(B x, A y1, A y2){}} new D().m(new B(), new B(), new B());");
		assertSuccess("class A{} class B extends A{} class C extends B{} class D{void m(B x, A y1, A y2){} void m(A x, A... y){assert false;}} new D().m(new B(), new B(), new B());");

		assertSuccess("class A {void m(int x, int y){assert true;} void m(float x, float y){assert false;}} new A().m(new Integer(1), 2);"); // priority autoboxing (box->primitive) over primitive cast
		assertSuccess("class A {void m(float x, float y){assert false;} void m(int x, int y){assert true;}} new A().m(new Integer(1), 2);"); // priority autoboxing (box->primitive) over primitive cast
		assertSuccess("class A {void m(Integer x, int y){assert true;} void m(float x, float y){assert false;}} new A().m(1, 2);"); // priority autoboxing(primitive->box) over primitive cast
		assertSuccess("class A {void m(float x, float y){assert false;} void m(Integer x, int y){assert true;}} new A().m(1, 2);"); // priority autoboxing(primitive->box) over primitive cast
		assertSuccess("class A {void m(long x, long y){assert true;} void m(float x, float y){assert false;}} new A().m(1, 2);"); // priority primitive cast over primitive cast
		assertSuccess("class A {void m(float x, float y){assert false;} void m(long x, long y){assert true;}} new A().m(1, 2);"); // priority primitive cast over primitive cast
		assertSuccess("class A {void m(long x, long y){assert true;} void m(char x, char y){assert false;}} new A().m(1, 2);"); // priority primitive cast over primitive cast
		assertSuccess("class A {void m(char x, char y){assert false;} void m(long x, long y){assert true;}} new A().m(1, 2);"); // priority primitive cast over primitive cast
		...
		assertSuccess("class A {void m(int x, int y){assert true;} void m(Integer x, Integer y){assert false;}} new A().m(1, 2);");
		assertSuccess("class A {void m(Integer x, Integer y){assert true;} void m(int x, int y){assert false;}} new A().m(new Integer(1), new Integer(2));");
		assertSuccess("class A {void m(Integer x, int y){assert true;} void m(int x, int y){assert false;}} new A().m(new Integer(1), 2);");
				...
				if (i1 != i2) {
					...
					for (int i = 0; i < types.length; i++) {
						...
						if (m1 || m2) {
							...
							assertSuccess("class A {void m(" + a1 + " x){assert " + ms1 + ";} void m(" + a2 + " x){assert " + ms2 + ";}} new A().m((" + a + ")1);");
						} else {
	...
	public void testNativeMethod() {
		...
		nativeObject(new A());
		assertSuccess("class A{native int m(int value);} assert new A().m(1) == 2;");
	...
	public void testConstructors() {
		// signature
		assertSuccess("class C{C(){}} new C();");
		assertSuccess("class A{A(int x, String arg){}} new A(0, \"a\"); new A(0, null);");
		assertSuccess("class C{C(int x, int y){assert false;} C(int x){}} new C(1);");
		...
		// varargs
		assertSuccess("class C{C(int length, int... n){assert n.length == length;}} new C(0); new C(3, 1, 2, 3);");
		assertSuccess("class O{} class O1 extends O{} class C{C(O... n){assert n.length == 2;}} new C(new O1(), new O());");
		assertSuccess("class O{} class C{Object[] o; C(Object... o){this.o = o;}} assert new C().o.length == 0; assert new C(new O(), \"x\", new Object()).o.length == 3;;");
		...
		assertFailCompile("class C{C(byte... x){}} new C(1, 2, 3);", //
				"constructor not found: C");
		assertSuccess("class C{C(int... x){assert false;} C(int x1, int x2, int x3){}} new C(1, 2, 3);");

		// this
		assertSuccess("class A{A(int x){assert x == 1;} A(int x, int y){this(x); assert y == 2;} } new A(1, 2);");
		assertSuccess("class A{A(int x){assert x == 1;} A(int x, int y){this(x); assert y == 2;} } new A(1, 2);");
		...
		// super
		assertSuccess("class A{A(){super();}} new A();");
		assertSuccess("class A{A(int x){this.x = x;} int x;} class B extends A{B(int y){super(y);}} A a = new B(1); assert a.x == 1;");
		assertSuccess("class A{A(int x, int y){}} class B extends A{B(int x, byte y){super(x, y);}} new B(1, (byte)1);");
	...
	public void testEnums() {
		assertSuccess("enum E{e1, e2, e3}; E e = E.e1; e = E.e2; if (e == E.e2) e = E.e3; assert e == E.e3; assert E.e3.ordinal() == 2;");
		assertSuccess("enum E1{e} enum E2{e;} assert E1.e != E2.e; assert E1.e.name().equals(E2.e.name()); assert !E1.e.equals(E2.e);");
		assertSuccess("enum E{e1(\"e1\", 0), e2(\"e2\", 1); String name; int number; E(String name, int number){this.name = name; this.number = number;} String getName(){return name;} int getNumber(){return number;}} E e = E.e2; assert e.getName().equals(\"e2\"); assert e.getNumber() == 1;");
		assertSuccess("enum E{e} assert E.e instanceof Enum; assert E.e instanceof E;");
	...
	public void testRecords() {
		assertSuccess("record Rec(int a); Rec r = new Rec(1);");
		assertSuccess("record Rec(int abc); Rec r = new Rec(1);");
		assertSuccess("class A{}; record Rec(A a, A b); new Rec(new A(), new A());");
		assertSuccess("record Rec(int a, String field); Rec rec = new Rec(1, \"abc\"); assert rec.getA() == 1; assert rec.getField().equals(\"abc\");");
		assertSuccess("record Rec(int a, int b){Rec(int a){this.a = a; b = 2;}}; Rec rec1 = new Rec(1, 2); Rec rec2 = new Rec(1); assert rec2.getA() == 1; assert rec2.getB() == 2;");
		assertSuccess("record Rec(int a, int b){int c; Rec(int c){a = 1; b = 2; this.c = c;} int getC(){return c;}}; Rec rec = new Rec(3); assert rec.getA() == 1; assert rec.getB() == 2; assert rec.getC() == 3;");

		// equals
		assertSuccess("record Rec(int a, String b); assert new Rec(1, \"a\").equals(new Rec(1, \"a\"));");
		assertFail("record Rec(int a, String b); assert new Rec(1, \"a\").equals(new Rec(2, \"a\"));", //
				"Assert failed");
		assertSuccess("record Rec(int a, String b); assert new Rec(1, \"abc\").equals(new Rec(1, \"abc\"));");
		assertSuccess("class O{} O o = new O(); record Rec(O o); assert new Rec(o).equals(new Rec(o));");
		assertFail("class O{} record Rec(O o); assert new Rec(new O()).equals(new Rec(new O()));", //
				"Assert failed");
		assertSuccess("class O{int a; O(int a){this.a = a;} public boolean equals(Object o){return a == ((O)o).a;}} record Rec(O o); assert new Rec(new O(1)).equals(new Rec(new O(1)));");

		// hashcode
		assertSuccess("class O{} record Rec(O o, int x, String a); new Rec(new O(), 1, \"x\").hashCode();");
	...
	public void testModifiers() {
		...
		assertFailCompile("private class A{};", //
				"modifier 'private' not allowed here");
		assertSuccess("class A{private class B{}};");
	...
	public void testAccess() {
		// fields access
		assertSuccess("class A{public int x = 1;} class B extends A{int y = x;} assert new B().y == 1;");
		assertSuccess("class A{public int x = 1;} A a = new A(); class B{int y = a.x;} assert new B().y == 1;");

		assertSuccess("class A{protected int x = 1;} class B extends A{int y = x;} assert new B().y == 1;");

		assertFailCompile("class A{private int x = 1;} class B extends A{int y = x;}", //
				"'x' has private access in 'A'");
		assertSuccess("class A{private int x = 1;} class B extends A{int y = super.x;} assert new B().y == 1;");
