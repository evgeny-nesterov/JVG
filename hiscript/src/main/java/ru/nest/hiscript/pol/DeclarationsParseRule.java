...
public class DeclarationsParseRule extends ParseRule<DeclarationsNode> {
		...
		if (type != null) {
			...
			if (variableName != null) {
				...
				while (visitSymbol(tokenizer, SymbolType.COMMA) != null) {
					if ((variableName = visitWord(WordType.NOT_SERVICE, tokenizer)) == null) {
	...
	public boolean visit(Tokenizer tokenizer, CompileHandler handler) {
		...
		if (type != null) {
			...
			String variableName = visitWord(WordType.NOT_SERVICE, tokenizer, handler);
			if (visitSymbol(tokenizer, handler, SymbolType.POINT) != null) {
				// TODO namespace = variableName;
				variableName = visitWord(WordType.NOT_SERVICE, tokenizer, handler);
			}
			...
			if (variableName != null) {
				...
				while (visitSymbol(tokenizer, handler, SymbolType.COMMA) != null) {
					if (visitWord(WordType.NOT_SERVICE, tokenizer, handler) == null) {
