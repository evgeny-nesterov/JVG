...
public class OperationDivide extends BinaryOperation {
	private static HiOperation instance = new OperationDivide();
	...
	public HiClass getOperationResultType(ValidationInfo validationInfo, CompileClassContext ctx, NodeExpressionNoLS.NodeOperandType node1, NodeExpressionNoLS.NodeOperandType node2) {
		...
		if (node2.isValue) {
			switch (t2) {
				case CHAR:
					if (((NodeChar) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
				case BYTE:
					if (((NodeByte) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
				case SHORT:
					if (((NodeShort) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
				case INT:
					if (((NodeInt) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
				case LONG:
					if (((NodeLong) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
				case FLOAT:
					if (((NodeFloat) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
				case DOUBLE:
					if (((NodeDouble) node2.node).getValue() == 0) {
						divisionByZero = true;
					}
					break;
			}
