 ...
 * контексте по типу. ƒл€ разных классов из разных контекстов, у которых имена совпадают, тип также может совпадать и, наоборот, дл€ разных типов
 * (например, — и A.B.C) класс может совпадать. ¬се зависит от того, в каком контексте производитс€ поиск класса по типу. ≈сли в коде указан
 * класс вместе с суперклассами (например, A.B.C), то дл€ данного класса создаетс€ тип с соответствующей иерархией (“ип —, —.parent = B, B.parent
 * = A, A.parent = null). ¬ данном случае вначале производитс€ поиск последнего суперкласса (A) по контексту, а потом в найденном классе
 * производитс€ поиск по дочернего элемента (в A ищетс€ B, а потом в B ищетс€ C).
 */
