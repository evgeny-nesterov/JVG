...
public class ClassFileParseRule extends ParseRule<Node> {
	...
	public Node visit(Tokenizer tokenizer, CompileContext ctx) throws TokenizerException, ParseException {
		...
		while (true) {
			...
			HiClass clazz = ClassParseRule.getInstance().visit(tokenizer, new CompileContext(compiler, null, HiClass.CLASS_TYPE_TOP));
			if (clazz != null) {
				continue;
			}

			HiClass interfaceClass = InterfaceParseRule.getInstance().visit(tokenizer, new CompileContext(compiler, null, HiClass.CLASS_TYPE_TOP));
			if (interfaceClass != null) {
				continue;
			}

			HiClass enumClass = EnumParseRule.getInstance().visit(tokenizer, new CompileContext(compiler, null, HiClass.CLASS_TYPE_TOP));
			if (enumClass != null) {
				continue;
			}

			HiClass recordClass = RecordParseRule.getInstance().visit(tokenizer, new CompileContext(compiler, null, HiClass.CLASS_TYPE_TOP));
			if (recordClass != null) {
				continue;
			}
			break;
